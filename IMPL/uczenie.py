...
NEW = True
# Wczytywanie danych
def create_dataset(img_folder, percent,max_count = 13000):
    i = 0
    img_data_array=[]
...
if NEW == True:
    ...
    model.add(layers.Conv2D(2*FIRST, (5, 5), activation='relu'))
    model.add(layers.MaxPooling2D(2,2))
    model.add(layers.Conv2D(4*FIRST, (20, 20), activation='relu'))
    ...
    model.add(layers.Dropout(0.4))
    model.add(layers.Dense(3*(250-19-2-19)**2, activation='relu', name = "dec"))
    model.add(layers.Reshape((IMG_HEIGHT-19-2-19, IMG_HEIGHT-19-2-19,3), name = "dec0"))
    model.add(layers.Conv2DTranspose(FIRST, (20, 20), activation='relu',name = "dec2"))
    model.add(layers.Conv2DTranspose(2*FIRST, (20, 20), activation='relu',name = "dec3"))
    model.add(layers.Conv2DTranspose(3, (3, 3), activation='sigmoid', name = "dec4"))
    
    #model.add(layers.Activation("sigmoid",name ="last"))
    #Kompilacja modelu
    run_opts = tf.compat.v1.RunOptions(report_tensor_allocations_upon_oom = True)
    model.compile(optimizer=Adam(lr=0.001), loss=keras.losses.MeanSquaredError(), options = run_opts)
else:
...
for i in range(TRAIN_EPOCHS):
    ...
    (train_images, train_labels), (test_images, test_labels) = create_dataset(IMG_FOLDER,0.8,np.min([100*(i+1),10000]))
    ...
    if(i%4 == 3):
        ...
        dec.compile(optimizer=Adam(lr=0.01), loss='mse', options = run_opts)
